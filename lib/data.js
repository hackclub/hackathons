...
export const getEvents = async (formula = 'TRUE()') => {
  ...
  let events = await getJSON(
    `https://api2.hackclub.com/v0.1/hackathons.hackclub.com/applications?select=${JSON.stringify(
      select
    )}`
  )
  events = events.map(
...
export const getEmailStats = async () => {
  ...
  console.log({
    all: subscribers.map(s => s.fields.parsed_country_code),
    unique: uniquify(subscribers.map(s => s.fields.parsed_country_code))
  })
