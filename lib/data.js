export const api_url_for = path => {
  const base =
    process.env.NEXT_PUBLIC_HACKATHONS_API_URL ||
    'https://dash.hackathons.hackclub.com/api/'
...
const api_get_all = async path => {
  ...
  const getPage = async url => {
    ...
    if (!next) return response.data

    return response.data.concat(await getPage(next))
  }

  return await getPage(api_url_for(path))
}
...
export const getEvents = async ({ upcoming } = { upcoming: false }) => {
  let events = await api_get_all('v1/hackathons')
